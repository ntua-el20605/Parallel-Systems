...
int main(int argc, char ** argv) {
    ...
    //----Rank 0 scatters the global matrix----//
        ...
        MPI_Scatterv(
            *U, // send buffer
            ...
            (*u_current+local[1]+3), // receive buffer
    ...
    if (rank_grid[0] == 0 && rank_grid[0] == grid[0] - 1) {
        i_min = 2;
        i_max = local[0] - local[0]*grid[0] + global[0];
    ...
    if (rank_grid[1] == 0 && rank_grid[1] == grid[1] - 1) {
        j_min = 2;
        j_max = local[1] - local[1]*grid[1] + global[1];
    ...
    MPI_Waitall(count_neighbours, req, MPI_STATUSES_IGNORE);
    ...
    for (t=0;t<T;t++) {
                ...
                if (south_flag) {
                    MPI_Isend(*u_current + local[0]*(local[1]+2)+1, 1, exchange_row, south, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                    MPI_Irecv(*u_current+(local[0]+1)*(local[1]+2)+1, 1, exchange_row, south, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                }
                if (north_flag) {
                    MPI_Isend(*u_current + local[1]+3, 1, exchange_row, north, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                    MPI_Irecv(*u_current+1, 1, exchange_row, north, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                }
                if (east_flag) {
                    MPI_Isend(*u_current + local[1]+2 + local[1], 1, exchange_column, east, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                    MPI_Irecv(*u_current+local[1]+2 + local[1]+1, 1, exchange_column, east, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                }
                if (west_flag) {
                    MPI_Isend(*u_current+(local[1]+2)+1, 1, exchange_column, west, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                    MPI_Irecv(*u_current + local[1]+2, 1, exchange_column, west, 0, MPI_COMM_WORLD, &req[count_neighbours++]);
                }
        ...
        if (t%C==0) {
                ...
                MPI_Waitall(count_neighbours, req, MPI_STATUS_IGNORE);
                //************************************//
