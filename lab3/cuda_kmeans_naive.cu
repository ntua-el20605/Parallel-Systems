...
void find_nearest_cluster(int numCoords,
  ...
  /* Make sure the id is within bound for numObjects */
  if (tid < numObjs) {
    int index, i;
    ...
    /* objectID is the tid and clusterID is the index */
    ...
    if (deviceMembership[tid] != index) {
      ...
      /* (*devdelta) += 1.0; */
      // Use atomicAdd for thread-safe increment of devdelta
...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
  ...
  do {
    ...
    checkCuda(cudaMemcpy(deviceClusters, clusters, sizeof(double) * numClusters * numCoords,cudaMemcpyHostToDevice));
    ...
    /* TODO: Copy deviceMembership to membership
        checkCuda(cudaMemcpy(...)); */
    checkCuda(cudaMemcpy(membership, deviceMembership, sizeof(int) * numObjs, cudaMemcpyDeviceToHost));
